...
输入使用 Pydantic 模型；认证与 scope 校验由 backend/security/oauth.py 提供。
OpenAPI、Swagger UI 和 ReDoc 关闭。backend/api/router.py 仅保留兼容说明。
backend/main.py 既可作为 ASGI 模块导入，也可作为脚本直接启动 Uvicorn。

## 边界

API 层处理协议、输入验证、安全依赖和响应映射。文档检索由 DocumentStore
完成，模型调用由 backend/llm.py 完成。修改路由时同步 README 和集成测试。
